 ...
 * @param {boolean} shareDataBuffer  true: modiry origin data,  false: return new data.
 ...
 * @param {Number} nPower keyAging factor.  default 10 => 2**10 => repeat hash 1024 times.
 * @returns 
 */
export function encryptMessage(msg, key, nPower = 10) {
    ...
    const hmac = sha256.hmac( mainKey , msgBuffer)
...
export function decryptMessage(b64msg, key) {
    ...
    try {
        ...
        if( typeof pack === 'object'){
            console.log( pack )
            const hmac = sha256.hmac(mainKey, pack.msg)
