...
function ChaCha (key, iv) {
  if (key.byteLength < CHACHA_KEYSIZE) { throw new Error('key too short') }
  if (iv.byteLength < CHACHA_IVSIZE) { throw new Error('IV too short') }
  ...
  return function () {
    ...
    for (let i = 0; i < 16; i++) { outview.setUint32(i * 4, output[i] + state[i], true) }
    state[12]++
    if (state[12] == 0) {
      state[13]++
      if (state[13] == 0) {
...
export { ChaCha as chacha, CHACHA_IVSIZE, CHACHA_ROUNDS, CHACHA_KEYSIZE }
